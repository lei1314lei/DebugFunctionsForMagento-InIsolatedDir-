...
class Mage_Cron_Model_Observer
{
    ...
    protected function _generateJobs($jobs, $exists)
    {
        ...
        $scheduleAheadFor = \Martindebug\Config::cornScheduleAheadFor($scheduleAheadFor);

        $schedule = Mage::getModel('cron/schedule');

        $jobs= \Martindebug\Config::cronFilterJob($jobs);
        ...
        foreach ($jobs as $jobCode => $jobConfig) {
            ...
            $now = time();
            $now = \Martindebug\Config::cronJobScheduleStartTime($now);
